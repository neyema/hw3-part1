...
class NeuralNetMLP_2layers(BaseEstimator):
    ...
    def backward(self, x, a_h1, a_h2, a_out, y):
        ...
        # Part 3
        d_z_h2__a_h1 = self.weight_h2  # [n_hidden2, n_hidden]
        # output dim: [n_examples, n_hidden]
        d_loss__a_h1 = np.dot(delta_2, d_z_h2__a_h1)
        # [n_examples, n_hidden]
        d_a_h1__d_z_h1 = a_h1 * (1. - a_h1)  # sigmoid derivative
        delta_1 = d_loss__a_h1 * d_a_h1__d_z_h1  # "delta (rule) placeholder"
        d_z_h1__d_w_h1 = x  # [n_examples, n_hidden]
        # output dim: [n_hidden2, n_hidden]
        d_loss__d_w_h1 = np.dot(delta_1.T, d_z_h1__d_w_h1)
        d_loss__d_b_h1 = np.sum(delta_1, axis=0)
